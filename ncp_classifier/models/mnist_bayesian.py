...
from ncp_classifier.models.mnist_utils import (
        generate_partial_mnist,
        generate_od_data,
        get_batches
        )
...
batch_size = 100
display_step = 1
# scaling of logits output before the entropy is calculated,
# to reduce the size of the gradients
NCP_SCALE = 1
...
def run_single(digits_to_omit, ood_transformations, alpha, experiment_suffix):
    """
    Run a single experiment, i.e. train one network to
    completion on one dataset
    """
    logging = dict()
    ...
    id_loss, id_ncp_loss, id_logits, id_ncp_std = network_tpl(id_data)
    od_loss, od_ncp_loss, od_logits, od_ncp_std = network_tpl(od_data)
    ...
    with tf.Session() as sess:
        ...
        od_images, od_labels = generate_od_data(
                id_images,
                id_labels,
                ood_transformations,
                plot=False)
