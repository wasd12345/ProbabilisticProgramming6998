    ...
    if EXPERIMENT_NAME == 'alpha' or EXPERIMENT_NAME == 'digout':
        ...
        for ds in digouts:
            ...
            for nn, a in enumerate(alphas):
                plt.vlines(
                        a,
                        id_entropy_mean[nn] - id_entropy_std[nn],
                        id_entropy_mean[nn] + id_entropy_std[nn],
                        colors='b',
                        alpha=opacity)
                plt.vlines(
                        a,
                        od_entropy_mean[nn] - od_entropy_std[nn],
                        od_entropy_mean[nn] + od_entropy_std[nn],
                        colors='g',
                        alpha=opacity)
                plt.vlines(
                        a,
                        om_entropy_mean[nn] - om_entropy_std[nn],
                        om_entropy_mean[nn] + om_entropy_std[nn],
                        colors='r',
                        alpha=opacity)
            ax0.xaxis.set_major_formatter(FormatStrFormatter('%g'))
            ...
            fig.savefig(os.path.join(
                savedir,
                f'experiment_entropy{ds}.pgf'),
                bbox_inches='tight')
            fig.savefig(os.path.join(
                savedir,
                f'experiment_entropy{ds}.pdf'),
                bbox_inches='tight')
            # =============================================================================
            # Accuracies
            # =============================================================================
            ...
            fig.savefig(os.path.join(
                savedir,
                f'experiment_accuracy{ds}.pgf'),
                bbox_inches='tight')
            fig.savefig(os.path.join(
                savedir,
                f'experiment_accuracy{ds}.pdf'),
                bbox_inches='tight')
